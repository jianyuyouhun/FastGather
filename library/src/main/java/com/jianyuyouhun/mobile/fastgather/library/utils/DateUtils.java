...
public class DateUtils {
    ...
    public static long getToDayTime() {
        ...
        String toDay = getYMDByMilliseconds(currentTime);
        return getTimeMillisecondsByYMD(toDay);
    }

    public static String getCurrentDateTime() {
        return getDateStringByMillisecondsWithFormatString(System.currentTimeMillis(), "yyyy-MM-dd HH:mm:ss");
    }

    public static String getMsgShownTime(long timeMilliseconds) {
        long currentTimeMillis = System.currentTimeMillis();
        if (isSameDay(currentTimeMillis, timeMilliseconds)) {
            long duration = currentTimeMillis - timeMilliseconds;
            if (duration < MINUTE) {//小于一分钟
                return "刚刚";
            }
            if (duration < DAY) {
                return getDateStringByMillisecondsWithFormatString(timeMilliseconds, "HH:mm");
            }
        }
        return getDateStringByMillisecondsWithFormatString(timeMilliseconds, "yyyy-MM-dd");
    }
}
